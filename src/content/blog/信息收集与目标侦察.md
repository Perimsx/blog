...
tags:
  - 渗透测试(预攻/后渗透)
description: 渗透测试信息收集阶段完整方法论，Nmap全端口扫描与脚本引擎实战、被动侦察工具链整合、子域名枚举爆破与DNS区域传送、资产发现与指纹识别，帮助构建全面目标画像
coverImage: ./covers/sec023-cover.webp
url: sec023
---
> 阅读提示
> 适合对象：渗透测试入门者和需要系统化整理侦察流程的读者。
> 阅读前提：了解域名、端口、常见服务和命令行环境。
> 核心收获：把被动侦察、主动探测和结果整理串成完整流程。
> 说明：扫描参数与工具输出会随版本变化，授权范围内操作前请先确认目标边界。
