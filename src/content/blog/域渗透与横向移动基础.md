...
tags:
  - Windows安全/域渗透
  - 渗透测试(预攻/后渗透)
description: Active Directory域渗透入门到进阶，BloodHound攻击路径自动化分析与可视化，Kerberos认证协议攻击原理剖析（ASREPRoast、Kerberoasting、GoldenTicket等），域内横向移动技术实战与防御策略
coverImage: ./covers/sec031-cover.webp
url: sec031
---
> 阅读提示
> 适合对象：在学习 AD 域环境攻击面和横向移动路径的安全研究者。
> 阅读前提：理解 Kerberos、域控、用户组与基本 Windows 管理概念。
> 核心收获：把枚举、路径分析、凭据利用与横向移动串联起来。
> 说明：AD 组件版本和补丁状态会影响具体利用路径。
