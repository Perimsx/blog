...
tags:
  - 日志与流量分析
description: tcpdump与Wireshark深度应用指南，从过滤器语法到协议分析技巧，实战演示HTTP/DNS/TCP/TLS流量特征识别
coverImage: ./covers/sec010-cover.webp
url: sec010
---
> 阅读提示
> 适合对象：需要用 tcpdump 和 Wireshark 做排障、取证和告警分析的读者。
> 阅读前提：了解 TCP/IP、常见端口和基础抓包概念。
> 核心收获：提升抓包、过滤、重放和流量还原的实战效率。
> 说明：不同版本的过滤表达式和界面细节可能存在差异。
